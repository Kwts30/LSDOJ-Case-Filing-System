...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const totalDocuments = await db.collection('generated_documents').countDocuments();
    const totalLogs = await db.collection('activity_logs').countDocuments();

    // Get documents by type
    const documentsByType = await db.collection('generated_documents').aggregate([
      { $group: { _id: '$documentType', count: { $sum: 1 } } },
      { $sort: { count: -1 } }
    ]).toArray();

    // Get recent documents
    const recentDocuments = await db.collection('generated_documents')
      .find()
      .sort({ createdAt: -1 })
      .limit(10)
      .toArray();
...
router.get('/documents', async (req, res) => {
  try {
    ...
    const total = await db.collection('generated_documents').countDocuments();
...
router.get('/reports', async (req, res) => {
  try {
    ...
    // Documents by type
    const docsByType = await db.collection('generated_documents').aggregate([
      { $group: { _id: '$documentType', count: { $sum: 1 } } }
    ]).toArray();

    // Documents by user
    const docsByUser = await db.collection('generated_documents').aggregate([
      { $group: { _id: '$userId', count: { $sum: 1 }, clientName: { $first: '$clientName' } } },
      { $sort: { count: -1 } }
    ]).toArray();

    // Documents by date (last 30 days)
    const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
    const docsByDate = await db.collection('generated_documents').aggregate([
...
router.get('/analytics', async (req, res) => {
  try {
    ...
    // User activity
    const userActivity = await db.collection('activity_logs').aggregate([
      { $group: { _id: '$userId', actions: { $sum: 1 } } },
      { $sort: { actions: -1 } }
    ]).toArray();

    // Login activity
    const logins = await db.collection('activity_logs')
      .countDocuments({ action: 'login' });

    // Document generation activity
    const documentCreations = await db.collection('activity_logs')
...
router.get('/export/report', async (req, res) => {
  try {
    ...
    if (type === 'documents') {
      const documents = await db.collection('generated_documents').find().toArray();
